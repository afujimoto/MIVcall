...
#				my $seq = substr($l[9], $rpos1, $rpos2 - $rpos1);
				my $seq = substr($l[9], $rpos1, $rpos2 - $rpos1 + 1); #220524
...
#				print"$l[0]\t$l[1]\t$cigar\t$gpos1->$rpos1\t$gpos2->$rpos2\t$seq_len\t$seq\t$ave_q1\t$ave_q2\t$l[9]\t$_\n";
					my $res_tmp = "$l[0]\t$l[1]\t$cigar\t$gpos1->$rpos1\t$gpos2->$rpos2\t$seq_len\t$seq\t$ave_q1\t$ave_q2\t$l[9]\t$_\n";
					push(@result, $res_tmp);
				}
				else{
				}
		...
		print"$contig\t$gpos1\t$gpos2\t($MS_type)n\t$seq\t";
#		foreach(sort {$a <=> $b} keys %num){print"$_;$num{$_}\t";}
		my @read_num;
		foreach(sort {$a <=> $b} keys %num){my $tmp = "$_".";"."$num{$_}";push(@read_num, $tmp);}
		print join(",", @read_num), "\n";
#		print"\n";
		@result = ();
	}
