...
if(! $READ_F or ! $RMSK){print STDERR $message.LF; exit(0);}

my %option_hash = ("I" => "$READ_F", "REF" => $bwa_ref, "MQ" => $mq_cutoff, "LL" => $len_cutoff1, "ML" => $len_cutoff2, "FL" => $flanking_len_cutoff, "SL" => $S_length_cutoff, "BQ" => $q_score_cutoff, "SW" => $SW_alignment, "GO" => $d, "GE" => $e);
...
my $cmd = "perl $SRC/GPOS2RPOS.BLOOD.READ_F.pl $option_str";
#print"$cmd\n";
system("$cmd");
