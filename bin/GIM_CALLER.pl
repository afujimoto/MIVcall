...
my $blood_length_num_col = 5;
my $unit_col = 3;

my ($infile, $blood_depth_cutoff, $blood_L_cutoff, $error_rate_file, $min_VAF, $min_num);
GetOptions(
	"I=s" => \$infile,
	"D=i" => \$blood_depth_cutoff,
	"L=f" => \$blood_L_cutoff,
...
while(<IN>){
	...
	my $blood_genotype = join("/", @blood_genotype);

	my $L_second_allele = "NA";
	my $second_allele = "NA";
	if(exists($blood_genotype2{$blood_genotype[0]}) == 1 && $blood_genotype2{$blood_genotype[0]} != 1){
		$L_second_allele = $blood_genotype2{$blood_genotype[0]};
		$second_allele = $blood_genotype[0];
	}
	elsif(exists($blood_genotype2{$blood_genotype[1]}) == 1 && $blood_genotype2{$blood_genotype[1]} != 1){
		$L_second_allele = $blood_genotype2{$blood_genotype[1]};
		$second_allele = $blood_genotype[1];
	}
	...
	my $VAF = &round($blood_read_num{$second_allele}/$blood_total_depth, 3);

	if($blood_genotype[0] eq $blood_genotype[1]){
		print"$_\t$blood_genotype\t-\n";
	}
	else{
		if($VAF >= $min_VAF and $blood_read_num{$second_allele} >= $min_num){
			print"$_\t$blood_genotype\tminor_allele=$second_allele;L=$L_second_allele;Number=$blood_read_num{$second_allele};VAF=$VAF\n";
		}
		else{
			my $major_allele = "";
			if ($blood_genotype[0] eq $second_allele){$major_allele = $blood_genotype[1];}
			if ($blood_genotype[1] eq $second_allele){$major_allele = $blood_genotype[0];}
			$blood_genotype = $major_allele."/".$major_allele;
			print"$_\t$blood_genotype\t-\n";
		}
	}
}
...
sub make_error_rate{
	...
	if($found_range == 0 || $found_type == 0){
		die "Range or MS type did not find in the Error rate matrix!! $MS_type, $MS_range_length";
	}
	else{
		my %error_rate;
